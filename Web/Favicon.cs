            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(iconUrl))
                {
                    ...
                    if (Uri.TryCreate(iconUrl, UriKind.RelativeOrAbsolute, out faviconUrl))
                    {
                        ...
                        try { icon = Image.FromStream(dataStream); }
                        catch (ArgumentException) { } // Thrown if data is not a valid image format
                    }
                }
            }
            catch (WebException /*ex*/)
            {
                // Throwing disabled here as for hltv.org a connection closed error was thrown which then caused not checking the secondary URLs
